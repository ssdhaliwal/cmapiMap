define(["esri/dijit/Legend"], function(esriLegend) {
    ...
    var Legend = function(map) {
        ...
        me.handleLegendPopOut = function() {
            $('#basemaps_wrapper, #overlay_wrapper, #about_wrapper, #batchselect_wrapper, #draw_wrapper, #edit_wrapper, #bookmark_wrapper, #measure_wrapper, #quickIcon_wrapper').hide();
            $('#basemaps, #overlay, #about, #batchselect, #draw, #bookmark, #measure, #edit, #quickIcon').removeClass('selected');

            $('#legend').toggleClass('selected');

            //change handler to close the legend
            $("#legend").on('click', handleLegendClose);
            ...
            $('.legend_divider').mousedown(function(e){
                e.preventDefault();
        ...
        var handleLegendClose = function() {
            ...
            $('#overlay_wrapper, #about_wrapper, #batchselect_wrapper, #basemaps_wrapper, #draw_wrapper, #edit_wrapper, #bookmark_wrapper, #measure_wrapper, #quickIcon_wrapper').hide();
            $('#basemaps, #overlay, #about, #batchselect, #legend, #draw, #bookmark, #measure, #edit, #quickIcon').removeClass('selected');

            $("#legend").on('click', me.handleLegendPopOut);
        };
        ...
        var setLegendWidth = function(width) {
            $("#legend_wrapper").width(width);
            $(".legend_divider").css('left', width+'px');
            if(width > 0) {
                $('.esri_info_div').width(width + legendDividerWidth);

                $('#legend_data').width(width - 20);
                $(".legend_divider").width(3);
            } else {
                $('.esri_info_div').width(0);
                $('#legend_data').width(0);
                $(".legend_divider").width(0);
            }
        };
        ...
        var handleLayout = function() {
            var windowWidth = $(window).width();
            setMapWidth(windowWidth - ($("#legend_wrapper").width() + $(".legend_divider").width()));
        };
    };
