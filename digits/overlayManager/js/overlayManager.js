 ...
 */
define(["app/extensions/cmwapi-adapter", "esri/layers/FeatureLayer"], function (cmwapiAdapter, FeatureLayer) {
    var adapter;
    var infoNotifier;

    /**
     * When the Overlay Manager is instantiated it will create the overlay tree, update it with
     * the current values for overlays and features from the then bind to all events
     ...
     * @param infoNotifier {module:notifier}
     * @alias module:digits/OverlayManager
     */
    var OverlayManager =  function(payload, errorNotifier, notifier) {
        var me = this;

        infoNotifier = notifier;
        this.adapter = adapter = me.adapter = new cmwapiAdapter(payload, errorNotifier, infoNotifier);
        
        //The manager html is populated from a template @ /digits/overlayManager/index.html
        $('#overlay_wrapper').load('./digits/overlayManager/index.html', function() {
            ...
            $('#overlay-tree').tree({
                ...
                onCreateLi: function(node, $li) {
                    ...
                    switch(node.format)
                    {
                        ...
                        case "arcgis-imageservicevector":
                        case "arcgis-tiledmapservice":
                            imageIcon = 'ISL.png';
                            break;
                        case "graphic":
                            imageIcon = 'Draw.png';
                    ...
                    if (node.parent.isHidden == null) {
                        var checked = node.isHidden === false ? (inputString + '" checked="checked"/>') : (inputString + '"/>');
                    }
                    else {
                        var checked = node.isHidden === false && node.parent.isHidden === false ? (inputString + '" checked="checked"/>') : (inputString + '"/>');                        
                    }
    ...
    OverlayManager.prototype.toggleOverlayManager = function() {
        $('#basemaps_wrapper, #about_wrapper, #batchselect_wrapper, #draw_wrapper, #edit_wrapper, #bookmark_wrapper, #measure_wrapper, #quickIcon_wrapper').hide();
        $('#basemaps, #about, #batchselect, #legend, #draw, #bookmark, #measure, #edit, #quickIcon').removeClass('selected');
    ...
    var changeAddScrollState = function() {
        if($('#add-overlay-div').is(':visible') || $('#add-feature-div').is(':visible')) {
            ...
            $('#overlay_wrapper').css('height', ($('#overlay-manager-add').height() + 100));
        }
    };
    ...
    var changeTreeScrollState = function() {
        if(!$('#add-overlay-div').is(':visible') && !$('#add-feature-div').is(':visible')) {
            ...
            $('#overlay_wrapper').css('height',  managerHeightPadding);
        }
    };
    ...
    **/
    ...
    var resizeOverlayToTree = function(tree, offset) {
        ...
        $('#overlay_wrapper').css('height', (treeHeight + offset) + 'px');
    };
