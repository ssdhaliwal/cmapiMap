define([], function() {
    var DataDiv = function(map) {
        ...
        var handleDataDivPopOut = function() {
            $('#basemaps_wrapper, #overlay_wrapper, #about_wrapper, #batchselect_wrapper, #draw_wrapper, #edit_wrapper, #bookmark_wrapper, #measure_wrapper').hide();
            $('#basemaps, #overlay, #about, #batchselect, #legend, #draw, #bookmark, #measure, #edit').removeClass('selected');
        ...
        var setLegendHeight = function(height) {
            $(".legend_divider").height(height);
            $("#legend_wrapper").height(height);
            $('.esri_info_div').height(height);
            if(height === '100%') {
                $('#legend_data').css('max-height', height);
            } else {
                $('#legend_data').css('max-height', height+'px');
            }
        };
