define([
    ...
    "dojo/_base/url",
    "dojo/_base/lang"
], function (declare, Graphic, GraphicsLayer, InfoTemplate, PopupTemplate, graphicsUtils, Color, SimpleMarkerSymbol,
        SimpleLineSymbol, SimpleFillSymbol, SimpleRenderer, SpatialReference, webMercatorUtils, Point, Polyline, Polygon, esriRequest, esriConfig, Url, lang
    ) {
    return declare([GraphicsLayer], {
        ...
        constructor: function (options) {
            ...
            // Default popup
            if (!options.popupTemplate || !options.popupTemplate.hasOwnProperty("fieldInfos")) {
                // find first item in collection and get properties if defined
                if (options.geojson.features) {
                    if (options.geojson.features.length > 0) {
                        if (options.geojson.features[0].hasOwnProperty("properties")) {
                            var propKeys = Object.keys(options.geojson.features[0].properties);
                            var fieldInfos = [];
                            ...
                            var newPopupTemplate = new PopupTemplate({  
                              title: "Attributes",  
                              fieldInfos: fieldInfos  
                            });  
                            this.setInfoTemplate(newPopupTemplate);
                        }
                    }
                }

                //var newPopupTemplate = new popupTemplate("GeoJSON Data", "${*}");
                //this.setInfoTemplate(newPopupTemplate);
            } else {
                this.setInfoTemplate(options.popupTemplate);
            }
            // Renderer
            if (options.renderer) {
        ...
        _setDefaultSymbols: function () {
            ...
            this._simpleLineSym = new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,new Color([255, 0, 0]),3);
        ...
        _getGeoJson: function (geojson) {
            ...
            if (geojson.type == "GeometryCollection") {
                //GeometryCollections not handled well by the parser so we will parse it ourselves. 
                for (var i in geojson.geometries) {
                    switch (geojson.geometries[i].type) {
                        case "Point":                            
                            var point = new Point(geojson.geometries[i].coordinates, this._inSpatialReference);
                            var pointGraphic = new Graphic(point, this._getEsriSymbol(point.type));                            
                            this._addGraphicToLayer(pointGraphic);                            
                            break;
                        case "MultiPoint":
                            var pCoordinates;
                            var mpCoordinates = geojson.geometries[i].coordinates;
                            for (j in mpCoordinates) {
                                pCoordinates = mpCoordinates[j];
                                var point = new Point(pCoordinates);
                                point.setSpatialReference(this._inSpatialReference);
                                var mpGraphic = new Graphic(point, this._getEsriSymbol("point"));
                                this._addGraphicToLayer(mpGraphic);
                            }
                            break;
                        case "LineString":
                            var coordinates = geojson.geometries[i].coordinates;
                            var line = new Polyline(coordinates);
                            line.setSpatialReference(this._inSpatialReference);
                            var lineGraphic = new Graphic(line, this._getEsriSymbol(line.type));
                            this._addGraphicToLayer(lineGraphic);
                            break;
                        case "MultiLineString":
                            var lCoordinates;
                            var mlCoordinates = geojson.geometries[i].coordinates;
                            for (j in mpCoordinates) {
                                lCoordinates = mlCoordinates[j];
                                var line = new Polyline(lCoordinates);
                                line.setSpatialReference(this._inSpatialReference);
                                var mlGraphic = new Graphic(line, this._getEsriSymbol("polyline"));
                                this._addGraphicToLayer(mlGraphic);
                            }
                            break;
                        case "Polygon":
                            var pCoordinates = geojson.geometries[i].coordinates;                            
                            var polygon = new Polygon(pCoordinates);
                            polygon.setSpatialReference(this._inSpatialReference);
                            var pGraphic = new Graphic(polygon, this._getEsriSymbol("polygon"));
                            this._addGraphicToLayer(pGraphic);
                            break;
                        case "MultiPolygon":
                            var pCoordinates;
                            var mpCoordinates = geojson.geometries[i].coordinates;
                            for (j in mpCoordinates) {
                                ...
                                var polygon = new Polygon(pCoordinates);
                                polygon.setSpatialReference(this._inSpatialReference);
                                var mpGraphic = new Graphic(polygon, this._getEsriSymbol("polygon"));
                                this._addGraphicToLayer(mpGraphic);
                            }
                            break;
                    }
                ...
                var arcgisJson = (arcgisJson.length > 0 ? arcgisJson : [arcgisJson]);
        ...
        _createGraphic: function (arcgisJson) {
            ...
            var fillSymbol = new SimpleFillSymbol("solid",
                    new SimpleLineSymbol("solid", new Color([50, 50, 50, 0.15]), 1),
                    getRandomColor("green", 0.15));
