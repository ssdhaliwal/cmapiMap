define(["esri/kernel", "esri/geometry/Extent", "esri/geometry/Point",
    "app/extensions/ViewUtils", "esri/tasks/PrintTask", "esri/tasks/PrintParameters", "esri/tasks/PrintTemplate"],
    function(EsriNS, Extent, Point, ViewUtils, PrintTask, PrintParameters, PrintTemplate) {
    /**
    ...
    var View = function(map, overlayManager) {
        ...
        */
        ...
        me.saveView = function() {
            ...
            var dataValue = JSON.stringify({bounds: bounds, center: center, range: range, zoom: zoom});
            /*
        ...
        me.setInitialView = function() {
            ...
            var successHandler = function(retValue) {
                if (retValue && retValue.value) {
                    viewDetails = JSON.parse(retValue.value);
                    // viewDetails.zoom = viewDetails.range;
                }

                // me.handleCenterBounds(null, viewDetails);
                me.handleCenterLocation(null, {location: {lon: viewDetails.center.lon, lat: viewDetails.center.lat}, zoom: viewDetails.zoom});

                map.on("extent-change", me.saveView);
                overlayManager.data.utilities.homeExtent = viewDetails;
            };
            ...
            var failureHandler = function(e) {
                ...
                */
                me.handleCenterLocation(null, {location: {lon: -76.809469, lat: 39.168101}, zoom: 10});
